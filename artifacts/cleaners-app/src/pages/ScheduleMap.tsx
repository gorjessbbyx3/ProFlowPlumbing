...
export default function ScheduleMap() {
  const [tab, setTab] = useState<"schedule" | "route" | "map">("schedule");
    ...
    <div className="space-y-4 pb-12">
      <div className="flex gap-2 bg-white rounded-2xl border border-border/60 p-1.5 shadow-sm w-fit">
        ...
        <button
          ...
          className={`flex items-center gap-2 px-4 py-2.5 rounded-xl text-sm font-bold transition-all ${
            tab === "route" ? "bg-primary text-white shadow-md" : "text-slate-600 hover:bg-slate-50"
          }`}
        >
      ...
      {tab === "schedule" ? <Scheduling /> : tab === "route" ? <RouteView /> : <MapView />}
    </div>
  );
}
